...
module.exports = function(db, table, fields, primary){
    ...
    function find(attrs, next){
        db.getConnection(function(err, connection){
            ...
            if(attrs){
                ...
                if(attrs.filter_col !== undefined && attrs.filter_col !== ''){
                    if(attrs.filter_txt !== undefined && attrs.filter_txt !== ''){
                        var filter_txt = attrs.filter_txt;
                        var filterArray = filter_txt.split(",");
                        for(var j = 0; j < filterArray.length; j++){
                            if(j==0){
                                sql += ` AND `+attrs.filter_col+` = '`+filterArray[j]+`'`;
                            }else{
                                sql += ` OR `+attrs.filter_col+` = '`+filterArray[j]+`'`;
                            }
                        }
                    }
                }
            ...
            connection.query(select_query, function(err, rows, fields){
                ...
                rows.forEach(function(row){
                    // console.log(modelfyRow(row).created_at.toISOString().replace(/T/, ' ').replace(/\..+/, ''));
    ...
    function createBatch(next){
        ...
        var column = attrs.key;
        var arr = attrs.value;
        countRows(function(err,result){
            var db_arr = result.map((e)=>e.tags);
            let difference = arr.filter(x => !db_arr.includes(x));
            var str = '';
            difference.forEach(function(i, index) {
                str += '("';
                str += i;            
                str += '")';
            ...
            if(str != ""){
                var insert_query = 'INSERT INTO  ' + table +' (`'+ column +'`) VALUES '+str;
                db.getConnection(function(err, connection){
    ...
    //             countRows(attr,attrs[value][attr], function(err,result){
